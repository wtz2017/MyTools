...
public class DashboardView4 extends View {
    ...
    private void init() {
        ...
        mColors = new int[]{mContext.getResources().getColor(R.color.dashboard_color_green),
                mContext.getResources().getColor(R.color.dashboard_color_yellow),
                mContext.getResources().getColor(R.color.dashboard_color_red)};
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 2017/12/6 修改 by wtz :把onMeasure中的部分代码移到此处
        mCenterX = mCenterY = getWidth() / 2f;
        mRectFArc.set(
                getPaddingLeft() + mStrokeWidth,
                getPaddingTop() + mStrokeWidth,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawColor(mContext.getResources().getColor(R.color.dashboard_color_dark));
        /**
         * 画圆弧
         */
        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setStrokeWidth(mStrokeWidth);
        mPaint.setColor(mContext.getResources().getColor(R.color.dashboard_color_dark_light));
         ...
         * 画指针
         */
        float θ = mStartAngle + mSweepAngle * (velocity - mMin) / (mMax - mMin); // 指针与水平线夹角
        mPaint.setColor(mContext.getResources().getColor(R.color.dashboard_color_dark_light));
        int r = mRadius / 8;
        canvas.drawCircle(mCenterX, mCenterY, r, mPaint);
        mPaint.setStrokeWidth(r / 3);
        mPaint.setColor(mContext.getResources().getColor(R.color.dashboard_color_light));
         ...
         * 画实时度数值
         */
        mPaint.setColor(mContext.getResources().getColor(R.color.dashboard_color_primary));
        mPaint.setStrokeWidth(dp2px(2));
        int xOffset = dp2px(22);
        if (velocity >= 100) {
            drawDigitalTube(canvas, velocity / 100, -xOffset);
            drawDigitalTube(canvas, (velocity - 100) / 10, 0);
            drawDigitalTube(canvas, velocity % 100 % 10, xOffset);
        } else if (velocity >= 10) {
            drawDigitalTube(canvas, -1, -xOffset);
            drawDigitalTube(canvas, velocity / 10, 0);
            drawDigitalTube(canvas, velocity % 10, xOffset);
        } else {
            drawDigitalTube(canvas, -1, -xOffset);
            drawDigitalTube(canvas, -1, 0);
            drawDigitalTube(canvas, velocity, xOffset);
        }
    }
    ...
    /**
     * 不要随便改名字，外部类会用到此属性名字和其setter
     */
    private int velocity = mMin; // 实时速度
    ...
    public void setVelocity(int value) {
        if (velocity == value || value < mMin || value > mMax) {
            return;
        }
